...
const App: React.FC = () => {
  ...
  // Улучшенная синхронизация с реестром (Deep Merge)
  const syncToRegistry = (userData: UserProfile, avatarSeed: string) => {
    ...
    registry[userData.name] = {
      ...existingData, // Сохраняем историю сессий
      name: userData.name,
      xp: userData.xp,
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 selection:bg-rbt-red selection:text-white">
      ...
      <main className="flex-1 ml-72 p-8 min-h-screen relative flex items-center justify-center">
        <div className="max-w-6xl w-full">{renderContent()}</div>
      </main>
      <div className="fixed bottom-8 right-8 z-50">
         <button onClick={() => { setIsAuthenticated(false); localStorage.removeItem('rbt_academy_user'); }} className="w-14 h-14 bg-white shadow-xl rounded-full border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rbt-red transition-all hover:scale-110">
